...
function viewEmployeesMgr() {
    ...
    // get managers first
    connection.query('SELECT * FROM employee WHERE role_id=2', (err, managers) => {
        // if there's an error
        if (err) throw err;
        //console.log(people);
        // create object to place manager: employee: into
        managers.forEach(manager => {
            let tempName = manager.id + '. ' + manager.first_name + ' ' + manager.last_name;

            if(managerNames.indexOf(tempName) === -1) {
                managerNames.push(tempName);
            }
        });
                ...
                (err,list) => {
                      ...
                      if (list.length !== 0) {
                        list.forEach((item) => { 
                            ...
                            tableData[`${item.id}`] = `${name}`;
                        });
                      }
                      else {
                          tableData['None'] = 'No employees!';
                      }
